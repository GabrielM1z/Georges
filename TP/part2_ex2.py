...
def create_lost_packet_signal(signal, fs, taille_paquet, probabilite_perte, latence):
    nb = 0
...
def plot_signal_and_modified_signal(nb_p_plot, signal, temps, modified_signal, modified_temps):
    ...
    plt.title(f'Comparaison du Signal Original et du Signal avec perte ({probabilite_perte*100}%) DPCM')
    plt.legend()
    plt.show()
...
if __name__ == "__main__":
    ...
    probabilite_perte = 0.1 # probabilité que la latence se produise (1%)
    latence = 1  # Limite le temps de latence à 1 paquet 
    
    # Créer le signal modifié avec latence
    signal_paquet, temps_modifies = create_lost_packet_signal(signal, fs, taille_paquet, probabilite_perte, latence)
    
    # Tracer le signal original et le signal modifié
    plot_signal_and_modified_signal(5, signal, np.arange(len(signal)) / fs, signal_paquet, temps_modifies)
    
#    Convertir le signal en format audible
    signal_int16 = np.int16((signal_paquet + 1) * 32767 / 2)
    
    # Enregistrer le signal audio modifié
    save_modified_audio(signal_int16, fs, f'TP/signaux/dpcm_perte_{probabilite_perte}_signal.wav')